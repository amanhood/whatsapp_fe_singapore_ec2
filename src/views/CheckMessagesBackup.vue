...
async function selectAccount(){
  ...
  contacts_list.value = []
  hasMore.value = true
  // 1. get team customers for this account
  await getTeamsWithCustomers()
  // 2. load contact list (logic inside will check team_customers)
...
const displayContacts = computed(() => {
  return (team_customers.value && team_customers.value.length > 0)
...
const handleScroll = (e) => {
  ...
  if (bottomReached && hasMore.value) {
    if(team_customers.value.length != 0 ){
      getTeamsWithCustomers()
    } else {
      getContactList()
    }
  }
    ...
    <div class="messenger-sidebar" v-show="!isMobile || !selected_customer">
      <div class="messenger-sidebar-header">
        ...
        <div class="d-flex align-items-center flex-1 ps-3">
          ...
          <button v-if="team_customers.length > 0"
