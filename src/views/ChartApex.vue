...
import apexchart, { getApexConfiguration } from '@/assets/components/plugins/Apexcharts.vue';
	...
	methods: {
		...
		getChart1Options() {
			return {
				chart: {
					...
					shadow: {
						enabled: true,
						color: 'rgba('+ appVariable.color.bodyColorRgb + ', .5)',
						top: 18,
						left: 7,
						blur: 10,
						opacity: 1
				...
				dataLabels: {
					...
					background: {
						enabled: true,
						foreColor: appVariable.color.bodyColor,
						padding: 4,
						borderRadius: 2,
						borderWidth: 0,
						borderColor: appVariable.color.bodyColor,
						opacity: 0.9,
						dropShadow: {
							enabled: false,
							top: 1,
				...
				grid: {
					row: { colors: [appVariable.color.light, 'transparent']}
				},
				markers: { size: 4 },
		...
		getChart4Options() {
			return {
				...
				xaxis: {
					...
					axisTicks: { show: true, borderType: 'solid', color: appVariable.color.borderColor,	height: 6, offsetX: 0, offsetY: 0 }
				}
			};
		},
		getChart5Options() {
		...
		getChart7Options() {
			return {
				chart: { type: 'bubble' },
				dataLabels: { enabled: false },
				colors: [appVariable.color.theme, appVariable.color.warning, 'rgba('+ appVariable.color.bodyColorRgb + ', .5)', appVariable.color.pink],
				fill: { opacity: 0.8 },
		...
		getChart8Options() {
			return {
				...
				colors: [appVariable.color.theme, appVariable.color.warning, 'rgba('+ appVariable.color.bodyColorRgb + ', .5)'],
				xaxis: {
					tickAmount: 10,
					labels: {
		...
		getChart10Options() {
			return {
				...
				stroke: { show: false },
				colors: [ 'rgba('+ appVariable.color.pinkRgb +', .75)',  'rgba('+ appVariable.color.warningRgb +', .75)',  'rgba('+appVariable.color.themeRgb +', .75)', 'rgba('+ appVariable.color.bodyColorRgb + ', .5)',  'rgba('+appVariable.color.indigoRgb +', .75)'],
			...
			return {
				...
				plotOptions: {
					radialBar: {
						...
						track: { background: 'rgba('+ appVariable.color.bodyColorRgb + ', .25)' },
			...
			return {
				...
				plotOptions: {
					radar: {
						size: 140,
						polygons: {
							strokeColors: 'rgba('+ appVariable.color.bodyColorRgb +', .25)',
							strokeWidth: 1,
							connectorColors: 'rgba('+ appVariable.color.bodyColorRgb +', .25)',
							fill: {
								colors: ['rgba('+appVariable.color.gray300Rgb +', .25)', 'rgba('+ appVariable.color.bodyColorRgb + ', .25)']
							}
						}
					}
				},
