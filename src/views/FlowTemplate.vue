...
function selectButton(){
    if(buttons.value.length < 3){
      ...
      } else if (selected_button_type.value.id == "flow"){
        custom_button_shown.value = true
        buttons.value.push({'id':button_id.value,'type':selected_button_type.value.id,flow_id:'',flow_name:'',text:''})
        ...
        if(buttons.value.length > 0){
          ...
          buttons.value.forEach((item) => {
            ...
            if(item.type=='quick_reply'){
              ...
              buttons_components.push({"type": "FLOW","flow_id": item.flow.id,"text":item.text})
            }
          })
          data['component'].push({
              "type":'buttons',
...
async function submitForm(payload){
  ...
  let data = await postRequest("generate_flowtemplate",payload,token)
      ...
      <div class="col-md-12">
        ...
        <div class="row" v-if="custom_button_shown" v-for="button in buttons" style="margin-bottom:10px;">
          ...
          <template v-if="button.type == 'flow'">
            <div class="col-md-3">
              <v-select v-model="button.flow" :options="flows" label="value"></v-select>
            </div>
            <div class="col-md-3">
              <input type="text" class="form-control" placeholder="text" v-model="button.text"/>
            </div>
