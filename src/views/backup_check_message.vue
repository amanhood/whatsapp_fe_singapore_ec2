...
async function getConversations(contact){
    ...
    await nextTick();
    const container =
        document.querySelector('.messenger-content-body .ps') ||
        document.querySelector('.messenger-content-body .h-100');
...
onMounted(async() => {
        ...
        recomputeIsMobile()                             // NEW
        window.addEventListener('resize', recomputeIsMobile)  // NEW
        await nextTick(() =>{
...
onBeforeUnmount(() => {
    window.removeEventListener('resize', recomputeIsMobile) // NEW
...
.messenger-content-body {
  flex: 1 1 auto;
  min-height: 0;
  overflow: hidden;           /* PerfectScrollbar handles inner scroll */
}

.messenger-content-body .h-100 {
  height: 100%;
}

/* helps iOS/Android smooth scrolling inside the PS element */
