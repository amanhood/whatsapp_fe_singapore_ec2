...
import { exportResponsesToExcel } from '../composables/export_excel.js'
...
async function getSubmittedForms(){
	...
	let response = await postRequest("get_submitted_forms",payload,token)
	if(response['status'] == 200){
...
function selectedAnswer(answer){
    ...
    selected_answer.value.forEach((item) => {
      console.log(item)
    })
}
      ...
      <div class="row">
        <div class="col-md-12">
          ...
          <div class="row">
            ...
            <div class="col-md-3" v-if="selected_form">
              <button type="button" class="btn btn-yellow mb-1 me-1" @click="getSubmittedForms">Get submitted forms</button>
            </div>    
          </div>
        </div>
      </div>
    </card-body>
    <card-body class="pb-2" v-if="submitted_records.length > 0">
      <div class="row">
        <div class="col-md-12">
          <div class="row">
            <div class="col-md-3">
